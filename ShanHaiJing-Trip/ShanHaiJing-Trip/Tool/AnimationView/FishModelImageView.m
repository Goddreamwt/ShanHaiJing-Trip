...
@property(nonatomic) float newFromValue;
@end
...
-(UIImageView *)makeRotation:(UIImageView *)image speedX:(float)X speedY:(float)Y
{
    ...
    Y = -Y;
    animation.fromValue =[NSNumber numberWithFloat:self.newFromValue];

    if (X>0 && Y>0 ) {
        //象限1
        //默认是顺时针效果，若将fromValue和toValue的值互换，则为逆时针效果
        animation.toValue = [NSNumber numberWithFloat:-M_PI_4];
        self.newFromValue =-M_PI_4;
    }
    else if(X<0 && Y>0){
        //象限2
        animation.toValue = [NSNumber numberWithFloat:M_PI_4+M_PI];
        self.newFromValue =M_PI_4+M_PI;
        
    }
    else if(X<0 && Y<0){
        //象限3
        animation.toValue = [NSNumber numberWithFloat:M_PI_4+M_PI_2];
        self.newFromValue =M_PI_4+M_PI_2;
    }
    else if(X>0 && Y<0){
        //象限4
        animation.toValue = [NSNumber numberWithFloat:M_PI_4];
